...
\CAPstart{E}{n}~el siguiente capítulo de este \ac{TFM} se presentarán los conceptos y herramientas que es necesario comprender para poder seguir correctamente el flujo de este trabajo.
...
\textbf{Lenguajes} & HDL (Verilog, SystemVerilog) y herramientas de síntesis/implementación. & Lenguajes de alto o medio nivel (C, C++, Python, Java, etc.). \\ \hline
...
\begin{itemize}
    \item \textbf{Diseño lógico}: descripción del comportamiento con HDLs como Verilog, especificando registros, puertas lógicas, \ac{FSM}s, y protocolos de interconexión a utilizar.
...
Front-End & Diseño algorítmico y HLS; Diseño lógico RTL: Verilog, SystemVerilog; Reutilización de IP (Soft y Hard); Síntesis lógica a netlist \\ \hline
...
Diseño lógico RTL & Descripción detallada de la lógica interna, rutas de datos y control usando Verilog o SystemVerilog, lista para síntesis. \\ \hline
...
En la práctica, este método es ampliamente utilizado en el desarrollo \textit{hardware}. En el contexto de este trabajo, se ha realizado el desarrollo del diseño a nivel \textit{hardware} precisamente sobre un Block Design, por la cómoda integración y facilidad de uso dentro de Vivado Design Suite.En la figura \ref{fig:bdVivado} se presenta un ejemplo de Block Design realizado con Vivado Design Suite
...
\begin{adjustwidth}{1.5cm}{}
\label{st:axiStream}
\textbf{AXI Stream} \vspace{0.25cm} \\
El protocolo \ac{AXI} Stream es una interfaz estándar de un solo sentido utilizada para la transmisión de datos de forma continua y eficiente entre componentes conectados, sin necesidad de direcciones o complejas fases de control \cite{arm-doc}. Constituye un enlace punto a punto entre un transmisor (\emph{master}) y un receptor (\emph{slave}), basado en una simple señalización de \textit{handshake} mediante un par de señales: 

\begin{itemize}
    \item \texttt{TVALID}: indica que el transmisor está enviando datos válidos.  
    \item \texttt{TREADY}: indica que el receptor está listo para recibir datos.  
\end{itemize}

Cuando se cumple la condición de que ambas señales están en alto en el mismo ciclo, se realiza la transferencia de datos por \texttt{TDATA} \cite{infoHandshakeAXI}. Para determinar si el dato a enviar es el último o no se tiene la señal \texttt{TLAST}, también empleada en este proyecto para implementar correctamente el flujo de los datos. Hay más señales, como \texttt{TKEEP}, \texttt{TSTRB}, \texttt{TID}, \texttt{TDEST} o \texttt{TUSER}, que permiten delimitar paquetes, identificar flujos o transmitir datos auxiliares \cite{amd-hls}, aunque estas últimas no tendrán relevancia en este proyecto.

El esquema básico del \textit{handshake} se presenta en \ref{fig:handshakeAXI}.

\begin{figure}[!ht]
  \centering
  \includegraphics[width=8cm]{figures/handshakeAXI.png}
  \caption{Handshake básico del protocolo AXI. Fuente: \cite{infoHandshakeAXI}}
  \label{fig:handshakeAXI}
\end{figure}
\end{adjustwidth}
...
\begin{adjustwidth}{1.5cm}{}
\label{st:obi}
\textbf{OBI} \vspace{0.25cm} \\
\ac{OBI} es un protocolo abierto, como su propio nombre indica, empleado en la arquitectura RISC-V, especialmente dentro del proyecto CORE-V del grupo OpenHW, para interconexiones punto a punto entre núcleos como el CV32E40* y módulos de memoria o periféricos \cite{infoOBI} \cite{ openhw-docs}.   
...
Las diferencias entre AXI-Stream y OBI se presentan en \ref{tbl:diferenciasAxiObi}

\begin{table}[h!]
\centering
  \begin{tabular}{|>{\bfseries}m{4cm}|m{5.5cm}|m{5.5cm}|}
    \hline
    ...
    \hline
    Señales clave & \texttt{TVALID}, \texttt{TREADY}, \texttt{TDATA}, opcionalmente \texttt{TLAST}, \texttt{TKEEP}, etc. & Señales de dirección, datos de lectura/escritura, tamaño de acceso (byte, media palabra, palabra). \\
    \hline
    ...
    \hline
  \end{tabular}
\caption{Comparación entre AXI-Stream y OBI}
\label{tbl:diferenciasAxiObi}
\end{table}
\end{adjustwidth}

\subsection{Herramientas utilizadas}

\subsubsection{X-HEEP y su extensión GR-HEEP}
\label{st:xheep}
\ac{X-HEEP} \cite{machetti2024xheep} \cite{xheepInfo} es una arquitectura de microprocesador de tipo RISC-V y de carácter\textit{open-source}, desarrollada en SystemVerilog por el \ac{EPFL}. Su diseño modular permite personalización de \textit{cores}, periféricos, memorias y topologías para habilitar aceleradores especializados sin modificar la plataforma base. X-HEEP puede implementarse sobre distintas tecnologías, incluyendo FPGAs, siendo ideal para este trabajo enfocado en el prototipado de microcontroladores RISC-V.

En este proyecto se utilizará una extensión llamada \ac{GR-HEEP} \cite{grheepInfo}, una extensión del \acf{SoC} base de X-HEEP. Concretamente, se empleará del repositorio de David Mallasén \cite{grHeepRepo}. GR-HEEP ofrece una plataforma más completa y preconfigurada, facilitando el uso de periféricos y aceleradores externos, sin embargo, actualmente está todavía en fase de prueba. Las principales diferencias entre X-HEEP y GR-HEEP se muestran en la tabla \ref{tbl:difsXheepGrheep}.

\begin{table}[h]
\centering
\begin{tabular}{|l|p{7cm}|p{6cm}|} \\
\hline
\textbf{Aspecto} & \textbf{X-HEEP} & \textbf{GR-HEEP} \\
\hline
\textbf{Naturaleza} & Plataforma base minimalista & Extensión preconfigurada de X-HEEP \\
\hline
\textbf{Desarrollador} & EPFL ESL Lab & VISI Lab, Universidad de Bolonia \\
\hline
\textbf{Propósito} & Exploración de aceleradores ultra-low-power & Prototipado rápido con periféricos adicionales \\
\hline
\textbf{Periféricos} & Básicos & Amplia gama (UART, I2C, SPI, etc.) \\
\hline
\textbf{Configuración} & Mediante \texttt{mcu-gen} y JSON & Hereda de X-HEEP con configuraciones adicionales \\
\hline
\end{tabular}
\caption{Comparación entre X-HEEP y GR-HEEP}
\label{tbl:difsXheepGrheep}
\end{table}

GR-HEEP mantiene la organización modular por dominios de energía (CPU, periféricos, memoria, always-on) y utiliza cores RISC-V de 32 bits estilo Harvard, sin caché, adecuados para aplicaciones de bajo consumo y RTOS integrados \cite{xheepDocs}. La figura \ref{fig:xheepArch} muestra la arquitectura de X-HEEP, sobre la cual se basa GR-HEEP. En este proyecto, se usará específicamente para prototipado en FPGA.

\begin{figure}[!ht]
\centering
\includegraphics[width=11cm]{figures/xheepArch.png}
\caption{Arquitectura de X-HEEP, sobre la que se basa GR-HEEP. Fuente: \cite{xheepInfo}}
\label{fig:xheepArch}
\end{figure}
...
\subsection{Sistemas Operativos de Tiempo Real (RTOS)}
\label{st:rtos}
Un RTOS es un tipo especializado de sistema operativo diseñado para gestionar tareas que deben ejecutarse dentro de límites de tiempo estrictos y predecibles. A diferencia de los sistemas operativos de propósito general, como Windows, que están orientados a la multitarea y la gestión eficiente de recursos, un RTOS se centra en garantizar que las tareas críticas se completen dentro de plazos específicos \cite{rtosInfo} \cite{rtosInfo_2}. Si bien hay multitud de opciones a día de hoy, destacan, entre otros, Zephyr \cite{zephyrInfo} y FreeRTOS \cite{freeRTOSInfo}, siendo este último el utilizado en este \ac{TFM}.

\subsubsection{FreeRTOS}
\label{st:freertos}
Originalmente desarrollado por Richard Barry en 2003, y mantenido por Amazon desde 2017, FreeRTOS es un \textit{kernel} de sistema operativo de tiempo real de código abierto, diseñado específicamente para sistemas embebidos con recursos limitados. Además, recientemente ha recibido soporte para RISC-V \cite{soporteRISCVFreeRTOS}, lo que lo hace idóneo para este proyecto. Las características principales de FreeRTOS son:
