...
qrun -batch -access=rw+/. '-timescale' '1ns/1ps' -mfcu design.sv testbench.sv '-voptargs=+acc=npr'  -do run.do  ; echo 'Creating result.zip...' && zip -r /tmp/tmp_zip_file_123play.zip . && mv /tmp/tmp_zip_file_123play.zip result.zip